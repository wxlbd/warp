...
const NOTICE_TITLE_KEY: &str = "workspace.free_ai_removal.notice.title";
const NOTICE_BODY_KEY: &str = "workspace.free_ai_removal.notice.body";
const NOTICE_BONUS_CREDITS_KEY: &str = "workspace.free_ai_removal.notice.bonus_credits";

const PROMPT_SUGGESTIONS_TITLE_KEY: &str = "workspace.free_ai_removal.prompt_suggestions.title";
const PROMPT_SUGGESTIONS_BODY_KEY: &str = "workspace.free_ai_removal.prompt_suggestions.body";
...
impl FreeAiRemovalModalVariant {
    fn title_key(self) -> &'static str {
        match self {
            Self::Notice => NOTICE_TITLE_KEY,
            Self::PromptSuggestions => PROMPT_SUGGESTIONS_TITLE_KEY,
        }
    }

    fn body_key(self) -> &'static str {
        match self {
            Self::Notice => NOTICE_BODY_KEY,
            Self::PromptSuggestions => PROMPT_SUGGESTIONS_BODY_KEY,
        }
    }
    ...
    fn secondary_key(self) -> Option<&'static str> {
        match self {
            Self::Notice => Some(NOTICE_BONUS_CREDITS_KEY),
    ...
    fn render_buttons(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let byok_button = appearance
            ...
            .with_centered_text_label(crate::localization::text_for_app(
                app,
                "workspace.free_ai_removal.action.byok",
            ))
            .build()
...
impl View for FreeAiRemovalModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = FormattedTextElement::from_str(
            crate::localization::text_for_app(app, self.variant.title_key()),
            font_family,
            18.,
        )
        ...
        let body = FormattedTextElement::from_str(
            crate::localization::text_for_app(app, self.variant.body_key()),
            font_family,
            14.,
        )
        ...
        if let Some(secondary_key) = self.variant.secondary_key() {
            let secondary = FormattedTextElement::from_str(
                crate::localization::text_for_app(app, secondary_key),
                font_family,
                14.,
            )
            .with_color(body_color)
