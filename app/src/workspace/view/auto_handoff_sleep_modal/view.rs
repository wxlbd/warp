...
impl AutoHandoffSleepModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let enable_button = ctx.add_view(|ctx| {
            ActionButton::new(
                localization::text_for_app(ctx, "workspace.auto_handoff_sleep_modal.action.enable"),
                PrimaryTheme,
            )
        ...
        let dismiss_button = ctx.add_view(|ctx| {
            ActionButton::new(
                localization::text_for_app(
                    ctx,
                    "workspace.auto_handoff_sleep_modal.action.dismiss",
                ),
                SecondaryTheme,
            )
            .with_full_width(true)
    ...
    fn render_title(app: &AppContext, appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            localization::text_for_app(app, "workspace.auto_handoff_sleep_modal.title"),
            appearance.ui_font_family(),
            20.,
        )
        .with_color(
    ...
    fn render_description(app: &AppContext, appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            localization::text_for_app(app, "workspace.auto_handoff_sleep_modal.description"),
            appearance.ui_font_family(),
            14.,
        )
        .with_color(
