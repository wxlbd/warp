...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let apply_code_diffs_dropdown_menu = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_items(
                vec![
                    ...
                    DropdownItem::new(
                        ai_settings_text(ctx, "settings.execution_profile.permission.always_allow"),
                        AISettingsPageAction::SetApplyCodeDiffs(ActionPermission::AlwaysAllow),
                    ),
                    DropdownItem::new(
                        ai_settings_text(ctx, "settings.execution_profile.permission.always_ask"),
        ...
        let mcp_allowlist_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_top_bar_max_width(AI_SETTINGS_DROPDOWN_WIDTH);
            dropdown.set_menu_width(AI_SETTINGS_DROPDOWN_WIDTH, ctx);
            dropdown.set_menu_header_to_static(ai_settings_text(
                ctx,
    ...
    fn create_grok_code_editor(ctx: &mut ViewContext<Self>) -> ViewHandle<EditorView> {
        ctx.add_typed_action_view(|ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text(
                ai_settings_text(ctx, "settings.ai.supergrok.placeholder.sign_in_code"),
                ctx,
            );
            editor
        })
    ...
    fn start_grok_oauth(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            Err(err) => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    ...
                    let toast = DismissibleToast::error(localization::text_for_app_with_args(
                        ctx,
                        "settings.ai.supergrok.toast.login_start_failed",
                        &[("error", error.as_str())],
                    ));
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                });
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            ...
            let toast = DismissibleToast::default(ai_settings_text(
                ctx,
                "settings.ai.supergrok.toast.opening_browser",
            ))
            .with_object_id(CONNECT_TOAST_OBJECT_ID.to_string())
            .with_link(
                ToastLink::new(ai_settings_text(
                    ctx,
                    "settings.ai.supergrok.action.copy_url",
                ))
            ...
            let toast = match result {
                Ok(tokens) => {
                    ...
                    DismissibleToast::success(ai_settings_text(
                        ctx,
                        "settings.ai.supergrok.toast.connected",
                    ))
                }
                Err(err) => {
                    me.grok_oauth_attempt = None;
                    ...
                    DismissibleToast::error(localization::text_for_app_with_args(
                        ctx,
                        "settings.ai.supergrok.toast.connect_failed",
                        &[("error", error.as_str())],
                    ))
                }
            };
                ...
                AgentModeCodingPermissionsType::iter()
                    .map(|t| {
                        let display = match t {
                            ...
                            AgentModeCodingPermissionsType::AlwaysAllowReading => ai_settings_text(
                                ctx,
        ...
        entries
            ...
            .map(|(pattern_clone, current_agent)| {
                ctx.add_typed_action_view(move |ctx| {
                    ...
                    let other_agent_header_label = other_agent_label.clone();
                    dropdown.set_menu_header_text_override(move |label| {
                        if label == other_agent_header_label {
            ...
            AISettingsPageAction::DisconnectGrokSubscription => {
                ...
                crate::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::default(
                        ai_settings_text(ctx, "settings.ai.supergrok.toast.disconnected"),
                    );
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                });
                ctx.notify();
...
impl SettingsWidget for UsageWidget {
    ...
    fn render(
        ...
        let request_limit_description = localization::text_for_app_with_args(
            app,
...
impl AgentsWidget {
    ...
    fn render_permissions_section(
        ...
        let code_diffs = self.render_execution_profile_dropdown(
            ...
            Icon::Code2,
            action_permission_description(code_diff_setting, app),
        ...
        read_files_flex.add_child(self.render_execution_profile_dropdown(
            ...
            Icon::Notebook,
            action_permission_description(read_files_setting, app),
        ...
        execute_commands_flex.add_child(self.render_execution_profile_dropdown(
            ...
            Icon::Terminal,
            action_permission_description(execute_commands_setting, app),
            ...
            .has_any_overrides()
        {
            widget_children.push(
                Container::new(render_settings_info_banner(
                    &ai_settings_text(app, "settings.ai.permissions.managed_by_workspace"),
                    None,
                    appearance,
                ))
                .with_margin_bottom(12.0)
        ...
        let write_to_pty = self.render_execution_profile_dropdown(
            ...
            Icon::Workflow,
            write_to_pty_permission_description(write_to_pty_setting, app),
    ...
    fn render_mcp_permissions_with_servers(
        ...
        let permission_setting = self.render_execution_profile_dropdown(
            localization::text_for_app(app, "settings.ai.agents.mcp.label"),
            Icon::Dataflow,
            action_permission_description(current_mcp_setting, app),
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let header = build_sub_header(
            appearance,
            ai_settings_text(app, "settings.mcp.page.title"),
            Some(styles::header_font_color(is_any_ai_enabled, app)),
        )
        .with_padding_bottom(HEADER_PADDING)
        .finish();
...
impl ApiKeysWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let grok_connect_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(
                ai_settings_text(ctx, "settings.ai.supergrok.action.connect"),
                SecondaryTheme,
            )
            .with_size(ButtonSize::Small)
            .on_click(|ctx| {
        ...
        let grok_connecting_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(
                ai_settings_text(ctx, "settings.ai.supergrok.action.connecting"),
                SecondaryTheme,
            )
            .with_size(ButtonSize::Small)
        });
        ...
        let grok_disconnect_button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new(
                ai_settings_text(ctx, "settings.ai.supergrok.action.disconnect"),
    ...
    fn render_grok_subscription_row(
        ...
        let label = Flex::row()
            ...
            .with_child(
                Text::new_inline(
                    ai_settings_text(app, "settings.ai.supergrok.label.use_your"),
                    appearance.ui_font_family(),
                    CONTENT_FONT_SIZE,
                )
                .with_color(text_color.into())
            ...
            .with_child(
                Text::new_inline(
                    ai_settings_text(app, "settings.ai.supergrok.label.subscription"),
                    appearance.ui_font_family(),
                    CONTENT_FONT_SIZE,
                )
                .with_color(text_color.into())
        ...
        let description = Container::new(
            Text::new(
                ai_settings_text(app, "settings.ai.supergrok.description"),
                appearance.ui_font_family(),
                CONTENT_FONT_SIZE,
            )
