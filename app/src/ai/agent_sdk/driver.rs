...
impl fmt::Display for AgentDriverError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            AgentDriverError::ManagedMcpResolutionFailed { uid, message } => text_with_args(
                "agent_sdk.driver.error.managed_mcp_resolution_failed",
                &[("uid", &uid.to_string()), ("message", message)],
            ),
            AgentDriverError::MCPStartupFailed { details } => {
                let message = text("agent_sdk.driver.error.mcp_startup_failed");
                if details.is_empty() {
                    message
                } else {
            ...
            AgentDriverError::BootstrapFailed { .. } => {
                text("agent_sdk.driver.error.bootstrap_failed")
            }
