...
pub(crate) fn rehydrate_codex_transcript(
    ...
    let sessions_root = codex_sessions_root().context(text(
        "agent_sdk.driver.harness.codex.error.resolve_sessions_root",
    ))?;
    let transcript_path = write_envelope(envelope, &sessions_root).context(text(
...
pub(crate) fn rehydrate_codex_transcript_from_reader(
    ...
    let envelope: CodexTranscriptEnvelope = serde_json::from_reader(reader).context(text(
        "agent_sdk.driver.harness.codex.error.parse_transcript_envelope",
    ))?;
    let session_id = envelope.session_id;
    let sessions_root = codex_sessions_root().context(text(
        "agent_sdk.driver.harness.codex.error.resolve_sessions_root",
