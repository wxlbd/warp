...
impl LoginSlideView {
    ...
    fn render_select_auth_bottom_nav(
        ...
        let skip_label_key = match self.login_purpose() {
            ...
            LoginPurpose::WarpAgent => "auth.disable_ai_features",
            LoginPurpose::ThirdParty => "auth.skip_for_now",
        };
        let skip_button = self.skip_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(
                    localization::text_for_app(app, skip_label_key).into(),
                ),
                theme: &button::themes::Naked,
                options: button::Options {
    ...
    fn render_skip_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let (title_key, body_key, features, cancel_label_key): (
            &'static str,
            &'static str,
            &'static [&'static str],
            &'static str,
