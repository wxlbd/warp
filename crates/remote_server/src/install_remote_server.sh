...
# The executable and its resources are siblings in the artifact. Prefer the
# channel-specific binary name (e.g. `warp-oss` for OSS); fall back to the
# historical `oz*` shape used by first-party CLI artifacts. Exclude the
